...
void FileHelperDecoder::InitDecoderSentence(int max_sentence_length) {
  max_sentence_length_ = max_sentence_length;
...
bool FileHelperDecoder::UseSentence(const std::vector<int> &v_input_sentence_int) {
  ...
  int current_tmp_source_input_index = 0;
  //while (iss_input_source >> word) {
  for (std::vector<int>::const_iterator iter = v_input_sentence_int.begin(); iter != v_input_sentence_int.end(); ++iter) {
    //v_tmp_input_sentence_source.push_back(std::stoi(word));
    p_host_input_vocab_indices_source_[current_tmp_source_input_index] = *iter;
    current_tmp_source_input_index += 1;
  }

  words_in_sentence_ = v_input_sentence_int.size();
  ...
  */
  ...
  eigen_minibatch_tokens_source_input_.resize(1, words_in_sentence_);
  sentence_length_ = words_in_sentence_;
  for (int i = 0; i < v_input_sentence_int.size(); ++i) {
    eigen_minibatch_tokens_source_input_(i) = v_input_sentence_int[i];
  }
