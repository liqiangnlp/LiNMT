...
bool DecoderSentence::GeneralSettings(std::map<std::string, std::string> &m_parameters) {
  ...
  key = "--multi-gpu";
  if (m_parameters.find(key) != m_parameters.end()) {
    std::vector<std::string> v_gpu_indices_str;
    basic_method_.Split(m_parameters[key], ' ', v_gpu_indices_str);
    for (int i = 0; i < v_gpu_indices_str.size(); ++i) {
      global_configuration_.gpu_indices_.push_back(std::atoi(v_gpu_indices_str[i].c_str()));
    }
    if (global_configuration_.gpu_indices_.size() != global_configuration_.model_names_.size()) {
...
/////////////////////////////////////////////////// DecoderSentenceTwoEncoders ///////////////////////////////////////////////////
bool DecoderSentenceTwoEncoders::Init(std::string &config) {
  logger<<"\n$$ Init\n";
  ...
  LoadParallelVocabulary(global_configuration_.model_names_[0], global_configuration_.target_vocab_size_);
  LoadAnotherEncoderVocabulary(global_configuration_.model_names_multi_source_[0]);
  
  global_configuration_.PrintDecodingSentParameters();
  
  ensemble_decode_.InitDecoderSentenceTwoEncoders(global_configuration_.model_names_, global_configuration_.model_names_multi_source_, \
                                                  global_configuration_.hypotheses_number_, global_configuration_.beam_size_, \
                                                  global_configuration_.min_decoding_ratio_, global_configuration_.penalty_, global_configuration_.longest_sentence_, \
                                                  global_configuration_.print_decoding_information_mode_, global_configuration_.print_alignments_scores_mode_, \
                                                  global_configuration_.decoder_output_file_, global_configuration_.gpu_indices_, \
                                                  global_configuration_.max_decoding_ratio_, global_configuration_.target_vocab_size_, \
...
bool DecoderSentenceTwoEncoders::Process(const std::string &input_sentence, const std::string &input_sentence_another_encoder, std::string &output_sentence) {
  std::vector<int> v_input_sentence_int;
  std::vector<int> v_input_sentence_another_encoder_int;
  IntegerizeSentence(input_sentence, v_input_sentence_int);
  IntegerizeSentenceAnotherEncoder(input_sentence_another_encoder, v_input_sentence_another_encoder_int);

  ensemble_decode_.DecodeSentenceTwoEncoders(v_input_sentence_int, v_input_sentence_another_encoder_int, output_sentence);
  UnintSentence(output_sentence);
...
bool DecoderSentenceTwoEncoders::ReadConfigFile(const std::string &config_file_name, std::map<std::string, std::string> &m_parameters) {
...
bool DecoderSentenceTwoEncoders::IntegerizeSentence(const std::string &input_sentence, std::vector<int> &v_input_sentence_int) {
...
bool DecoderSentenceTwoEncoders::IntegerizeSentenceAnotherEncoder(const std::string &input_sentence_another_encoder, std::vector<int> &v_input_sentence_another_encoder_int) {

  std::vector<std::string> v_input_tmp;
  basic_method_.SplitWithString(input_sentence_another_encoder, " |||| ", v_input_tmp);
  std::string input_sentence_tmp = v_input_tmp[0];

  std::istringstream iss_src(input_sentence_tmp, std::istringstream::in);
  std::string word;
  while (iss_src >> word) {
    if (uno_source_mapping_word_int_another_encoder_.count(word) == 0) {
      v_input_sentence_another_encoder_int.push_back(uno_source_mapping_word_int_another_encoder_["UNK"]);
    } else {
      v_input_sentence_another_encoder_int.push_back(uno_source_mapping_word_int_another_encoder_[word]);
    }
  }

  std::reverse(v_input_sentence_another_encoder_int.begin(), v_input_sentence_another_encoder_int.end());
  return true;
}


bool DecoderSentenceTwoEncoders::UnintSentence(std::string &sentence) {
...
bool DecoderSentenceTwoEncoders::GeneralSettings(std::map<std::string, std::string> &m_parameters) {
  ...
  key = "--another-encoder-vocab-mappings";
  if (m_parameters.find(key) != m_parameters.end()) {
    basic_method_.Split(m_parameters[key], ' ', global_configuration_.model_names_multi_source_);
  } else {
    logger<<"   Error: you must assign --another-encoder-vocab-mappings in config file!\n";
    exit(EXIT_FAILURE);
  }
  ...
  global_configuration_.training_mode_ = false;
  global_configuration_.decode_mode_ = false;
  global_configuration_.decode_sentence_mode_ = false;
  global_configuration_.decode_sentence_twoencs_mode_ = true;
  global_configuration_.test_mode_ = false;
  global_configuration_.stochastic_generation_mode_ = false;
  global_configuration_.postprocess_unk_mode_ = false;
  global_configuration_.calculate_bleu_mode_ = false;
...
bool DecoderSentenceTwoEncoders::LoadParallelVocabulary(const std::string &nmt_model_file, int &target_vocab_size) {
  std::ifstream in_nmt_model(nmt_model_file.c_str());
  if (!in_nmt_model) {
    logger<<"   Error: can not open "<<nmt_model_file<<" file!\n";
    exit(EXIT_FAILURE);
...
bool DecoderSentenceTwoEncoders::LoadAnotherEncoderVocabulary(const std::string &nmt_model_another_encoder_file) {
  std::ifstream in_nmt_model(nmt_model_another_encoder_file.c_str());
  if (!in_nmt_model) {
    logger<<"   Error: can not open "<<nmt_model_another_encoder_file<<" file!\n";
    exit(EXIT_FAILURE);
  }
  ...
  while (std::getline(in_nmt_model, str)) {
    ...
    // second is word
    iss >> word;
    uno_source_mapping_word_int_another_encoder_[word] = tmp_index;
  }
