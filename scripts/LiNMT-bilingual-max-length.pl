...
sub SetMaxLength
{
  ...
  open(TGTINFILE, "<:encoding(UTF-8)", $param{"-eng"}) or die "Error: can not read file $param{ '-eng' }.\n";
  open(SRCOUTFILE, ">:encoding(UTF-8)", $param{"-outchn"}) or die "Error: can not write file $param{'-outchn'}.\n";
  open(TGTOUTFILE, ">:encoding(UTF-8)", $param{"-outeng"}) or die "Error: can not write file $param{'-outeng'}.\n";
  if ($param{"-outfilter"} eq 1)
  {
    open(SRCOUTFILEFIL, ">:encoding(UTF-8)", $param{"-outchn"}.".filter") or die "Error: can not write file $param{'-outchn'}.filter.\n";
    open(TGTOUTFILEFIL, ">:encoding(UTF-8)", $param{"-outeng"}.".filter") or die "Error: can not write file $param{'-outeng'}.filter.\n";
  }
  ...
  while (my $line_src = <SRCINFILE>)
  {
    ...
    my @src_chars = split //, $line_src;
    my $out_src_str = "";
    foreach my $word (@src_chars)
    {
      $out_src_str .= " ".$word;
    }
    $out_src_str =~ s/\s+/ /g;
    ...
    my @src_chars_clean = split /\s+/, $out_src_str;
    #print SRCOUTFILE scalar(@src_chars_clean)."\n";

    my @tgt_words = split /\s+/, $line_tgt;
    #print TGTOUTFILE scalar(@tgt_words)."\n";
